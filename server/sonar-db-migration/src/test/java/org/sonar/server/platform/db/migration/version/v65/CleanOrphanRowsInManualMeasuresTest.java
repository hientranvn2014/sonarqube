...
public class CleanOrphanRowsInManualMeasuresTest {
  ...
  public void execute_deletes_rows_which_component_is_not_a_project_nor_a_view_nor_a_module_nor_a_subview() throws SQLException {
    Long[] validMeasureIds = {
      insertManualMeasure(insertComponent(SCOPE_PROJECT, QUALIFIER_PROJECT)),
      insertManualMeasure(insertComponent(SCOPE_PROJECT, QUALIFIER_VIEW)),
      insertManualMeasure(insertComponent(SCOPE_PROJECT, QUALIFIER_MODULE)),
      insertManualMeasure(insertComponent(SCOPE_PROJECT, QUALIFIER_SUBVIEW))
    };
    String invalidScope = randomAlphabetic(3);
    insertManualMeasure(insertComponent(invalidScope, QUALIFIER_PROJECT));
    insertManualMeasure(insertComponent(invalidScope, QUALIFIER_VIEW));
    insertManualMeasure(insertComponent(invalidScope, QUALIFIER_MODULE));
    insertManualMeasure(insertComponent(invalidScope, QUALIFIER_SUBVIEW));
    insertManualMeasure(insertComponent(SCOPE_PROJECT, "DIR"));
    insertManualMeasure(insertComponent(SCOPE_PROJECT, "FIL"));
    insertManualMeasure(insertComponent("DIR", "DIR"));
    insertManualMeasure(insertComponent("FIL", "FIL"));
    insertManualMeasure(insertComponent("FIL", "TRK"));
    insertManualMeasure(insertComponent(SCOPE_PROJECT, randomAlphabetic(3)));
    insertManualMeasure(insertComponent(SCOPE_PROJECT, randomAlphabetic(3)));
